...
const Inventory = () => {
    ...
    const [merks, setMerks] = useState(null);
    const [merkDialog, setMerkDialog] = useState(false);
    const [merkUpdateDialog, setMerkUpdateDialog] = useState(false);
    const [deleteMerkDialog, setDeleteMerkDialog] = useState(false);
    const [deleteMerksDialog, setDeleteMerksDialog] = useState(false);
    const [merk, setMerk] = useState(emptyMerk);
    ...
    const saveDataToApi = async () => {
        try { 
            const response = await createMerk(merk);
    ...
    const updateDataToApi = async () => {
        try { 
            const response = await updateExistingMerk(merk); // Gantilah dengan fungsi update yang sesuai
    ...
    const deleteMerk = async () => {
        try {
            await deleteMerkById(merk.id); // Use the renamed function to delete by id
            const _merks = merks.filter((val) => val.id !== merk.id);
            setMerks(_merks);
            setDeleteMerkDialog(false);
            setMerk(emptyMerk);
            ...
            toast.current.show({ severity: 'error', summary: 'Error', detail: 'Failed to delete merk', life: 3000 });
        }
    };
    ...
    const bulkDeleteSelectedMerks = async () => {
        try {
            const selectedMerkIds = selectedMerks.map((merk) => merk.id);
            await bulkDeleteMerks(selectedMerkIds);
            const _merks = merks.filter((merk) => !selectedMerkIds.includes(merk.id));
            setMerks(_merks);
            setDeleteMerksDialog(false);
            ...
            toast.current.show({ severity: 'error', summary: 'Error', detail: 'Failed to delete merks', life: 3000 });
        }
    };    
    ...
    const merkBodyTemplate = (rowData) => {
        return (
            <>
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    >
                        ...
                        <Column field="merk_name" header="Name" sortable body={nameBodyTemplate} headerStyle={{ minWidth: '10rem' }}></Column>
                        <Column field="created_by" header="Created By" sortable body={merkBodyTemplate}></Column>
                    ...
                    <Dialog visible={deleteMerkDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteMerkDialogFooter} onHide={hideDeleteMerkDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {merk && (
                                <span>
                                    Are you sure you want to delete <b>{merk.merk_name}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deleteMerksDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteMerksDialogFooter} onHide={hideDeleteMerksDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {merk && <span>Are you sure you want to delete the selected merks?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
