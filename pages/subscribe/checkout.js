...
const Checkout = () => {
        ...
        if (code) { // Memeriksa apakah ada kode yang diterima dari URL
            const fetchData = async () => {
                try {
                    ...
                    const response = await axios.get(`https://cashpay.my.id:8080/api/v1/package/${code}`, {
                        headers: {
    ...
    const initiatePayment = async () => {
        try {
            ...
            const response = await axios.post('https://cashpay.my.id:8080/api/v1/payment', paymentData, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
